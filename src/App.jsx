...
const Pill = ({ children, className = "" }) => (
  <span className={cls(
    "inline-flex items-center rounded-full border border-white/20 bg-white/5 px-3 py-1 text-xs text-white/80 backdrop-blur",
    className
  )}>{children}</span>
);
...
function Nav() {
  ...
  const LinkBtn = ({ id, children }) => (
    <button onClick={() => scrollToId(id)} className="hover:text-white">
      {children}
    </button>
  );
  return (
      ...
      {open && (
        <div className="md:hidden border-t border-white/10 bg-[#070815] px-4 py-3 text-white/80">
          {[
            ...
            <div key={id} className="py-1"><button onClick={() => { setOpen(false); scrollToId(id); }}>{label}</button></div>
          ))}
        </div>
      )}
    </header>
  ...
  return (
    <div className="relative h-44 w-full max-w-md">
      <svg viewBox="0 0 200 100" className="h-full w-full">
        ...
        <path d="M 10 50 C 40 10, 80 90, 100 50 C 120 10, 160 90, 190 50" stroke="url(#grad1)" strokeWidth="8" fill="none" strokeLinecap="round">
...
function RotatingCube() {
  return (
    <div className="relative h-28 w-28">
      <svg viewBox="0 0 100 100" className="h-full w-full">
        <g>
          <polygon points="50,10 80,25 80,55 50,70 20,55 20,25" fill="#0ea5e9" fillOpacity="0.25" stroke="#38bdf8" />
          <animateTransform attributeName="transform" attributeType="XML" type="rotate" from="0 50 40" to="360 50 40" dur="8s" repeatCount="indefinite" />
        </g>
      </svg>
    </div>
  );
  ...
  return (
    <div className="relative h-36 w-full max-w-lg">
      <svg viewBox="0 0 400 120" className="h-full w-full">
        ...
        <path d="M20,60 h80 a10,10 0 0 1 10,10 v0 a10,10 0 0 0 10,10 h60 a10,10 0 0 0 10,-10 v0 a10,10 0 0 1 10,-10 h80" stroke="url(#pipe)" strokeWidth="10" fill="none" strokeLinecap="round" />
        <circle r="5" fill="#a78bfa">
          <animateMotion dur="5s" repeatCount="indefinite" path="M20,60 h80 a10,10 0 0 1 10,10 v0 a10,10 0 0 0 10,10 h60 a10,10 0 0 0 10,-10 v0 a10,10 0 0 1 10,-10 h80" />
        </circle>
      </svg>
    </div>
  );
    ...
    <section id="home" className="relative overflow-hidden bg-[#070815] text-white">
      <Container className="relative grid min-h-[72vh] grid-cols-1 place-items-center gap-8 py-16 md:grid-cols-2">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-white/80 md:text-lg">Ravah.ai predicts risky changes, enforces quality gates, and gives you live & historical visibility across your CI/CD. Safer releases. Faster delivery.</p>
      ...
      <Container className="grid grid-cols-1 gap-8 md:grid-cols-2 items-center">
        <div>
          ...
          <div className="mt-6 grid grid-cols-2 gap-6 text-white/80 text-sm">
            <div>
              <p className="font-medium text-white">Predictive risk</p>
              <p>Flag risky commits before merge and block policy violations.</p>
            </div>
            <div>
              <p className="font-medium text-white">Quality gates</p>
              <p>Coverage, lint, test, SAST, size and ownership checks.</p>
            </div>
            <div>
              <p className="font-medium text-white">Live visibility</p>
              <p>See deploy health and MTTR in real time across services.</p>
            </div>
            <div>
              <p className="font-medium text-white">Beyond DORA</p>
              <p>Flow efficiency, MTTD/MTTI and SLO-aware guidance.</p>
            </div>
          </div>
        </div>
    ...
    <section id="pipeline" className="border-t border-white/10 bg-[#070815] py-16 text-white">
      <Container className="grid grid-cols-1 gap-8 md:grid-cols-2 items-center">
        <div>
          ...
          <p className="mt-3 text-white/70">Ravah’s Pipeline Copilot automates build, test, and deploy workflows with predictive AI. It identifies risky commits before merge, ensures compliance with quality gates, and provides real-time visibility into deployment health.</p>
        </div>
        <PipelineFlow />
      </Container>
    </section>
...
function PipelinePulseCalculator() {
  ...
  const InputSlider = ({ label, description, value, setValue, min, max, step, unit }) => (
    <div className="space-y-1">
      ...
      <input type="range" min={min} max={max} step={step} value={value} onChange={(e)=>setValue(Number(e.target.value))} className="w-full accent-teal-400"/>
